...
RemoteDoc.prototype.stringInsert = function(segments, index, value, cb) {
  ...
  if (previous == null) {
    ...
    if (util.isArrayIndex(lastSegment)){
      ...
      if (previous === undefined) {
        op = [new ListInsertOp(segments.slice(0, -1), lastSegment, value)];
      // null - there is an empty array item so we replace it
      } else {
        op = [new ListReplaceOp(segments.slice(0, -1), lastSegment, previous, value)];
      }
    } else {
      op = [new ObjectInsertOp(segments, value)];
    };
    this.shareDoc.submitOp(op, cb);
    this._updateCollectionData();
    return previous;
  };
  op = [new StringInsertOp(segments, index, value)];
  this.shareDoc.submitOp(op, cb);
  this._updateCollectionData();
  return previous;
};
