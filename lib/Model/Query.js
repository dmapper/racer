...
Query.prototype._diffMapIds = function(ids) {
  ...
  if (addedIds.length) {
    ...
    this._subscribeDocsPathQuery(addedIds);
  }

  if (removedIds.length){
    this._removeMapIds(removedIds);
    this._unsubscribeDocsPathQuery(removedIds);
  }
};

Query.prototype._subscribeDocsPathQuery = function(ids){
  if (!this.isPathQuery) return;
  for (var i = ids.length; i--;) {
    var id = ids[i];
...
Query.prototype._unsubscribeDocsPathQuery = function(ids){
  if (!this.isPathQuery) return;
  for (var i = ids.length; i--;) {
    var id = ids[i];
...
Query.prototype._setResults = function(results) {
  ...
  if (this.isPathQuery){
    ids = this.model._getCopy(this.expression) || [];
