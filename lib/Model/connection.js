...
Model.prototype._finishCreateConnection = function() {
  ...
  this.root.connection.on('doc', function(shareDoc) {
    // Ignore documents which the end user manages directly with ShareDB.
    // Those documents are in collections by the name prefixed with 'pure_'
    if (/^pure_/.test(shareDoc.collection)) return;
